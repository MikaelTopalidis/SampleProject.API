...
namespace SampleProject.UnitTests.Customers
{
    ...
    public class CustomerRegistrationTests : TestBase
    {
        ...
        [Test]
        public void GivenCustomerEmailIsUniqueButMailDoesNotContainNackadmeinDotSe_WhenCustomerIsRegistering_BreaksCustomerEmailMustBeNackademinRule()
        {
            // Arrange
            var customerUniquenessChecker = Substitute.For<ICustomerUniquenessChecker>();
            const string email = "testEmail@mail.se";
            customerUniquenessChecker.IsUnique(email).Returns(true);

            // Assert
            AssertBrokenRule<CustomerEmailMustBeNackademinRule>(() =>
            {
                // Act
                Customer.CreateRegistered(email, "Sample name", customerUniquenessChecker);
            });
